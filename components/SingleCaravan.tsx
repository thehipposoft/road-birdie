...
const GALLERY_CAPTION = 'Smart design inside.'
...
const TECHNICAL_SPECS: TechnicalSpec[] = [
  { label: 'Chassis', value: 'Mercedes Sprinter', icon: ChassisIcon },
  { label: 'Year', value: '1993 Edition', icon: YearIcon },
  { label: 'Connectivity', value: 'Starlink WiFi', icon: ConnectivityIcon },
  { label: 'Water', value: '20L Fresh Tank', icon: WaterIcon },
  { label: 'Bathing', value: 'Outdoor Shower', icon: BathingIcon },
  { label: 'Energy', value: 'Solar-Powered', icon: EnergyIcon },
  { label: 'Insurance', value: 'Full Coverage', icon: InsuranceIcon },
  { label: 'Capacity', value: 'Sleeps 2-4', icon: CapacityIcon },
]
...
const FEATURES: CaravanFeature[] = [
  { title: 'Equipped Kitchen', description: 'Induction hobs, espresso machine, and local artisan ceramic dinnerware.', icon: '/assets/images/svg/kitchen.svg' },
  { title: 'Off-Grid Ready', description: '400Ah lithium battery bank coupled with high-efficiency roof solar.', icon: '/assets/images/svg/battery.svg' },
  { title: 'Pick up From', description: 'Melbourne, Australia.', icon: '/assets/images/svg/pickup.svg' },
  { title: 'Comfortable Sleep', description: 'Custom-built memory foam mattress with organic bamboo linens.', icon: '/assets/images/svg/sleep.svg' },
  { title: 'Premium Comfort', description: 'Full soundproofing and wool insulation for quiet outback nights.', icon: '/assets/images/svg/comfort.svg' },
  { title: 'Unlimited Freedom', description: 'Unlimited kilometers and complimentary roadside assistance anywhere.', icon: '/assets/images/svg/kilometers.svg' },
]
...
export const SingleCaravan = ({ name, image }: SingleCaravanProps) => (
  <>
    ...
    <section className="w-full bg-white py-20">
      <div className="max-w-[1200px] mx-auto px-6">
        <h2 className="font-display text-5xl font-semibold text-main-black">{OVERVIEW.title}</h2>
        <p className="mt-6 max-w-xl font-body text-base text-light-gray">{OVERVIEW.description}</p>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6 items-start">
          <div className="relative h-125 rounded-2xl overflow-hidden">
            ...
            <div className="absolute inset-x-0 top-12 flex flex-col items-center gap-3 px-6 text-center">
              <p className="font-ui text-white/80 tracking-[0.4em] text-sm">
                {Array.from({ length: DECORATIVE_STARS }).fill('*').join(' ')}
              </p>
              <h3 className="font-display italic text-5xl leading-tight text-white">{GALLERY_CAPTION}</h3>
              <div className="mt-1 flex flex-col gap-1 w-32">
                <div className="h-px w-full bg-white/70" />
                <div className="h-px w-full bg-white/70" />
              </div>
            </div>

            <ArrowIcon className="absolute bottom-6 right-6 w-6 h-6 text-white" />
          </div>
          ...
          <div className="flex flex-col gap-6 bg-gray-50 py-6 px-8 rounded-xl border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-4">
              {TECHNICAL_SPECS.map(({ label, value, icon: Icon }) => (
                <div key={label} className="flex flex-col items-center gap-3 py-6 rounded-2xl bg-graybg/60 text-center">
                  ...
                  <p className="font-ui text-sm font-semibold text-main-black">{value}</p>
                </div>
              ))}
            </div>
